...
%% tensor linear MTL LSSVC
para.kernel='linear';
para.init_method='random';
para.method_name='tensor_mtl_lssvc';
para.save_path=['../results/SNR',num2str(SNR),'/'];
para.maxR=5;
para.chol=false;

s=tic;
[gamma_best, p_best, R_best] = tensor_GridMTLSSVC(trnX, trnY, trnN, para);
...
for rep=1:10
    s=tic;
    [alpha_L, U,b] = tensor_MTLSSVCTrain(trnX, trnY, trnN,para);
    Res_tensor.cpu_time(rep)=toc(s);
    [tmpY1, cfm{rep} ]= tensor_MTLSSVCPredict(tstX,tstY, tstN, trnX, trnN, trnY, para.kernel,alpha_L, U,b, p_best);
