function [alpha_L, U, b,loss,relative_error] = tensor_MTLSVRTrain(trnX, trnY, trnN, para)
...
switch init_method
    case 'given'
        if isfield(para,'Winit')
            ...
            Winit=obtain_good_initilaization_tsvr(trnX, trnY, trnN,taskinfo,para);
        end
...
while 1
    ...
    [alpha_L, b] = MTLSVRTrain_with_precomputed_Q(G_train_L, trnY,gamma,taskinfo,para);
    ...
    for d=1:D
        ...
        for t=1:T(d)
            ...
            alpha = MTLSVRTrain_with_precomputed_Q(G_train_U, trnY_sub,gamma,taskinfo_sub,para);
            U{d}(t,:)=alpha'*Z_t;      
        end
    end
...
function [svm_beta, b] = MTLSVRTrain_with_precomputed_Q(Omega, trnY,C,taskinfo,para)
%
%
M=size(trnY,1);
...
if  isfield(para,'cmp_method')
    method=para.cmp_method;
else
    method='cvx';
end
...
switch method
    case 'cvx'
        cvx_begin quiet
        variable svm_beta(M);
        minimize (0.5.*quad_form(svm_beta,Omega) - trnY'*(svm_beta)+epsilon.*ones(M,1)'*abs(svm_beta));
        subject to
        svm_beta >= -C;
        svm_beta <= C;
        A'*svm_beta==0;
    ...
    case 'quadprog'
        ...
        H=[Omega,Omega;Omega,Omega];
        f=-[trnY;trnY]+[epsilon.*ones(M,1);-epsilon.*ones(M,1)];
        Aeq=[A;A]';
        beq=zeros(T,1);
        lb=[zeros(M,1); -C.*ones(M,1)];
        ub=[C.*ones(M,1);zeros(M,1)];
        options=optimoptions('quadprog', 'Display','off');
        x = quadprog(H,f,[],[],Aeq,beq,lb,ub,[],options);
        svm_beta=x(1:M)+x(M+1:end);
        
end

tol = 1e-6;
sv_pos = find(svm_beta > tol & svm_beta < C - tol);
sv_neg= find(svm_beta > -C+tol & svm_beta < 0 - tol);
...
for t = 1: T
    ind=groups_t{t};
    idx_sv_pos=intersect(ind,sv_pos);
    
    if isempty(idx_sv_pos)
        idx_sv_neg=intersect(ind,sv_neg);
        b(t) = mean(trnY(idx_sv_neg) - Omega(idx_sv_neg,:)*(svm_beta)+epsilon);
        if isempty(idx_sv_neg)
            bt = (trnY(ind) - Omega(ind,:)*(svm_beta));
                b(t)=median((bt(svm_beta(ind)<C)));
        end
    else
        b(t) = mean(trnY(idx_sv_pos) - Omega(idx_sv_pos,:)*(svm_beta)-epsilon);
    end
